...
//this method brings posts for a source provider
    .controller('FeedEntriesCtrl', function ($scope, $stateParams, $http, FeedList, $q, $ionicLoading, PostService, BookMarkService) {
        $scope.feed = [];
        $scope.posts = [];
                ...
                FeedList.get(source.url)
                    .then(function (result) {
                        ...
                        PostService.getRecentPosts(1)
                            .then(function (data) {
                                var categoryMatched = [], tagsMatched = [];
                                $scope.totalPages = data.pages;

                                // Looking for matched category like na/EMEA etc
                                angular.forEach(data.posts, function(data){
                                        angular.forEach(data.tags, function(tags){
                                           if (tags.title === categoryId){
                                               categoryMatched.push(data);
                                           }
                                        });
                                });

                                // Once matched category found, filtering for matched tags
                                angular.forEach(categoryMatched, function(category){
                                    angular.forEach(category.tags, function(tags){
                                        // Removing white space from tags to find closest match
                                       if(tags.title.replace(/\s+/, "") === sourceId.replace(/\s+/, "")) {
                                            tagsMatched.push(category);
                                       }
                                    })
                                });
                                $scope.posts = PostService.shortenPosts(tagsMatched);
